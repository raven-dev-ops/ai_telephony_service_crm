...
@pytest.mark.skipif(
    get_settings().oauth.google_client_id is None, reason="OAuth clients not configured"
)
def test_google_and_linkedin_oauth_flow(monkeypatch):
    ...
    settings.oauth.linkedin_client_id = (
        settings.oauth.linkedin_client_id or "linkedin-id"
    )
    ...
    start_linkedin = client.get(
        "/auth/linkedin/start", params={"business_id": "biz-li"}
    )
